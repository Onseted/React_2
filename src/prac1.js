...
Link <-> useNavigate 
: link는 tag
: useNavigate는 hook method
---> 특정 조건에서 페이지 이동을 하려면 navigate. 
---> 프로젝트 내에서 단순 이동을 원한다면 Link. 

Params
: paramater는 <Route path에 선언된 :id값을 파라미터로 받겠다는 작동을 한다. 

Code Logic 
: db저장소로 보낸 값들을 refreshTopic을 통해 local project환경인 topics에 저장한다. 
: 새로운 create가 발생하면, 이를 또한 refresh한다. 

useEffect 
: (함수, 객체(혹은 배열)을 통해 구동 되며, 두 번째 파라미터가 있다면 그 값이 변경될 때마다 실행함을 의미. 
*/